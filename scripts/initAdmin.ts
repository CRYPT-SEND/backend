...
async function initAdminCollection() {
  try {
    ...
    if (!adminSnapshot.empty) {
      console.log(`L'administrateur ${testAdminData.email} existe déjà dans Firestore.`);
    } else {
      ...
      try {
        ...
        console.log(`Utilisateur créé dans Firebase Auth avec UID: ${userRecord.uid}`);
        ...
        console.log(`Admin créé avec succès dans Firestore avec ID: ${userRecord.uid}`);
    ...
    console.error('Erreur lors de l\'initialisation de la collection admin:', error);
    process.exit(1);
  }
}
