...
/**
 * Les méthodes login, register, refreshToken, logout sont gérées côté 
 * backend pour orchestrer l'appel au service.
 * Le backend valide le token, gère l'autorisation et peut enrichir le 
 * profil utilisateur.
 */

/**
 * Côté frontend :
 * - L'utilisateur se connecte avec le SDK Firebase 
 *   (email/mot de passe, Google, etc.).
 * - Le frontend reçoit un idToken (JWT) après connexion.
 * - À chaque requête API, le frontend envoie ce idToken dans 
...
/**
 * Les méthodes login, register, refreshToken, logout ne sont plus 
 * nécessaires côté backend.
